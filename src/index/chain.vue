...
*/
<template>
  <div id="chain" class="map-container">
    <a href="javascript:;" id="back">返回上级</a>
    <div v-show="!mode" id="heatBar">
      <div class="item type1"></div>
      <div class="item type2"></div>
      <div class="item type3"></div>
      <div class="item type4"></div>
      <div class="item type5"></div>
    </div>
  </div>
</template>

<script>
const CHINA_JSON = require('@/assets/public/three/china.json')
class ChainWorld {
  static flowData
  config
  scene
  ...
  constructor (mapData, flowData, config) {
    this.setConfig(config)
    this.mapData = mapData
    this.flowData = Object.assign({ children: [], name: '', totalFlow: 0 }, flowData)
  ...
  init () {
    ...
    this.drawLightBar(this.config.lightBarData)
    this.render()
  ...
  setInitDom () {
    let item = `
      <div id="province">${this.flowData.name}</div>
      <div id="store"><span>${this.flowData.totalFlow || 0}</span>家分店</div>
    `
  ...
  drawMap (data, scale, group, lineGroup, center) {
    ...
    this.vector3json.forEach(provinces => {
      ...
      this.flowData.children.forEach(data => {
  ...
  getAreaMesh (points, province) {
    ...
    let material = new THREE.MeshBasicMaterial({
      color: this.config.mode ? 'rgb(255, 255, 255)' : 'rgb(28, 30, 54)',
      transparent: true,
      opacity: 1,
        ...
        if (mesh.name === intersects[0].object.name) {
          ...
          if (intersects[0].object.amount) {
            ...
            if (this.lastCoverMeshName) {
              ...
              let item = `
              <div id="province">${this.flowData.name}</div>
              <div id="store"><span>${this.flowData.totalFlow}</span>家分店</div>
              `
              this.chainEl.innerHTML = item
            }
          }
  ...
  setConfig (data) {
   this.config = Object.assign({ lightBarData: [], mode: 1 }, data)
  }

  getProvinceMap (id, center, name) {
    (async () => {
      let data = await require(`@/assets/public/three/geometryProvince/${id}.json`)
      this.provinceId = id
  ...
  getCityMap (id, center) {
    (async () => {
      let country = await require(`@/assets/public/three/geometryCouties/${id + (id.length === 6 ? '' : '00')}.json`)
      this.setInitDom()
      let scale = 2800
      this.status = 'city'
      this.resetGroup()
      this.drawMap(country, scale, this.cityGroup, this.cityLineGroup, center)
    })()
  }
...
export default {
  ...
  computed: {
    mode () {
      return parseInt(this.getQueryVariable('mode'))
    }
  },
  methods: {
    ...
    handleMessage (event) {
      ...
      switch (data.cmd) {
        case 'post_country_data':
          (async () => {
            geoData = await CHINA_JSON
            this.flowData = Object.assign({ children: [], name: '', totalFlow: 0 }, data.data)
            console.log(_this.flowData, geoData)
            this.flowData.children.forEach(item => {
              // this.lightBarData.push({name: item.province})
            })
            this.world = new ChainWorld(geoData, this.flowData, { mode: this.mode, lightBarData: this.lightBarData })
            this.world.init()
          })()
          break
        case 'post_province_data':
          this.world.flowData = data.data
          this.world.getProvinceMap(data.id, data.center, data.name)
          break
        case 'post_city_data':
          this.world.flowData = data.data
          this.world.getCityMap(data.id, data.center, data.name)
          break
      }
    }
  },
